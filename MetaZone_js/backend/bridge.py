    ...
    def _loop():
        while True:
            ...
            try:
                ...
                pass
            if batch:
                # Single evaluate_js call for the whole batch collected
                # in this tick, instead of one call per event -- cuts
                # IPC call count under load, per the "avoid excessive
                # IPC messages" requirement.
                ...
                try:
                    ...
                    # yet. Re-queue the batch (in original order, at
                    # the front) instead of dropping it or crashing the
                    # drain thread -- the next tick retries once the
                    # page has finished setting up its event listener.
